(() => {
  ...
  const API_URL = window.KB_HELP_API_URL || "https://formspree.io/f/xgojpond";
  ...
  widget.innerHTML = `
    ...
    <button class="kb-help-launcher" type="button" aria-label="Segítség megnyitása" aria-expanded="false" aria-controls="kbHelpPanel">
      <span class="kb-help-launcher__icon" aria-hidden="true">?</span>
      <span class="kb-help-launcher__label">Segítség</span>
    </button>
    <aside class="kb-help-panel" id="kbHelpPanel" role="dialog" aria-modal="true" aria-labelledby="kbHelpTitle" aria-hidden="true">
  ...
  const setOpen = (open) => {
    ...
    panel.inert = !open;
    document.documentElement.classList.toggle("kb-help-is-open", open);
    if (open) {
      lastFocus = launcher;
      requestAnimationFrame(() => {
        if (panel.dataset.open === "true") closeBtn.focus();
      });
  ...
  const showView = (name, moveFocus = true) => {
    let activeView = null;
    widget.querySelectorAll("[data-view]").forEach((el) => {
      el.hidden = el.dataset.view !== name;
      if (!el.hidden) activeView = el;
    });

    if (moveFocus && activeView) {
      const preferredTarget = activeView.querySelector("textarea, input:not([type=hidden]), .kb-help-action, .kb-help-back, a[href]");
...
if (document.body.classList.contains("home-page")) {
  ...
  currentScript.src = "js/home-current.js?v=c58a168520c2";
