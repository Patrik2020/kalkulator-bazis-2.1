...
const structuredData = JSON.stringify({
  ...
  "@graph": [
    ...
    },
    {
      ...
      "@id": `${hubCanonical}#calculator`,
      name: "Átlag kalkulátor központ",
      applicationCategory: "CalculatorApplication",
      operatingSystem: "Web",
      url: hubCanonical,
      description: "Számtani, súlyozott és mértani átlag számítása egy közös eszközben.",
      offers: { "@type": "Offer", price: "0", priceCurrency: "HUF" },
      isPartOf: { "@id": "https://kalkulatorbazis.hu/#website" },
      about: "Mindennapi kalkulátorok",
    },
    {
      "@type": "BreadcrumbList",
...
<section class="hero">
  <p class="eyebrow">Három átlagfajta, egy helyen</p>
  <h1>Átlag kalkulátor – számtani, súlyozott és mértani átlag</h1>
  <p>Válaszd ki, milyen átlagot szeretnél számolni. Az eszköz az egyszerű számlistát, az eltérő súlyokat és a pozitív növekedési tényezőket külön kezeli.</p>
</section>
<section class="card card-calculator kb-calculator-shell" id="kalkulator">
  <div class="everyday-heading">
    <h2>Átlagszámítás</h2>
    <p>A számokat pontosvesszővel, szóközzel vagy sortöréssel válaszd el. Tizedesvessző használható.</p>
  </div>
  <div class="everyday-grid">
    <label class="everyday-field" for="averageMode"><span>Számítási mód</span>
      <select id="averageMode">
        <option value="simple">Számtani átlag + medián</option>
        <option value="weighted">Súlyozott átlag</option>
        <option value="geometric">Mértani átlag</option>
      </select>
    </label>

    <div data-average-panel="simple">
      <label class="everyday-field" for="averageValues"><span>Értékek</span>
        <textarea id="averageValues" rows="6">12; 15; 18; 21</textarea>
        <small>Azonos jelentőségű értékekhez. Az eredmény mellett mediánt, összeget és elemszámot is mutatunk.</small>
      </label>
    </div>

    <div data-average-panel="weighted" hidden>
      <label class="everyday-field" for="weightedValuesHub"><span>Értékek</span>
        <textarea id="weightedValuesHub" rows="4">80; 95; 70</textarea>
      </label>
      <label class="everyday-field" for="weightedWeightsHub"><span>Súlyok</span>
        <textarea id="weightedWeightsHub" rows="4">2; 3; 1</textarea>
        <small>Minden értékhez ugyanazon a pozíción egy nem negatív súly tartozzon.</small>
      </label>
    </div>

    <div data-average-panel="geometric" hidden>
      <label class="everyday-field" for="geometricValuesHub"><span>Pozitív értékek vagy növekedési tényezők</span>
        <textarea id="geometricValuesHub" rows="5">1,05; 0,98; 1,12</textarea>
        <small>A mértani átlaghoz minden értéknek pozitívnak kell lennie. +5% például 1,05, −3% pedig 0,97.</small>
      </label>
    </div>
  </div>
...
<section class="adsense-content calculator-guide">
  ...
  <h2>Mikor félrevezető az egyszerű átlag?</h2>
  <p>Ha az adatok eltérő súlyúak, a számtani átlag túl- vagy alulértékelheti a fontosabb elemek hatását. Egymást követő százalékos változásoknál pedig a hatások nem összeadódnak, hanem összeszorzódnak, ezért ott a mértani szemlélet lehet megfelelőbb.</p>
  <p>A számtani módban ezért a mediánt is megmutatjuk. Ha az átlag és a medián nagyon eltér, érdemes megnézni, nincs-e néhány szélsőséges adat, amely elhúzza az átlagot.</p>

  <h2>Példák</h2>
  <ul>
    <li><strong>Számtani:</strong> 10, 20 és 30 átlaga 20.</li>
    <li><strong>Súlyozott:</strong> egy kétszeres súlyú eredmény kétszer akkora hatással vesz részt a végeredményben.</li>
    <li><strong>Mértani:</strong> +5%, −2% és +12% növekedési tényezői 1,05; 0,98; 1,12 formában adhatók meg.</li>
  </ul>

  <h2>Korlátok</h2>
  <p>Az átlag önmagában nem mutatja meg az adatok szóródását, eloszlását vagy ok-okozati kapcsolatát. Statisztikai, pénzügyi vagy tudományos döntésnél az átlagot mindig az adatok jelentésével és további mutatókkal együtt értelmezd.</p>
...
const replaceMetaProperty = (html, property, value) => {
...
const transformHub = (html) => {
  ...
  if (!html.includes("../js/atlag-kozpont.js")) {
    html = html.replace(/<\\/body>/i, `<script defer src="../js/atlag-kozpont.js"></script>\n</body>`);
  }
  return html;
};

const retirementSchema = JSON.stringify({
  "@context": "https://schema.org",
  "@graph": [
...
const retirementNotice = `<!-- KB_PHASE2:retired:START -->\n<section class="info-box phase2-retired-calculator">\n  <strong>Ez az önálló átlagoldal kivezetés alatt van.</strong>\n  A számtani, súlyozott és mértani átlagot egy közös Átlag kalkulátorba vontuk össze.\n  <a href="atlag-kalkulator">Nyisd meg az Átlag kalkulátor központot.</a>\n</section>\n<!-- KB_PHASE2:retired:END -->`;

const transformRetired = (html) => {
  html = replaceMetaName(html, "robots", "noindex,follow");
  html = replaceCanonical(html, hubCanonical);
  html = replaceStructuredData(html, retirementSchema);
...
for (const page of retired) write(page, transformRetired(read(page)));

const siteDataPath = "js/site-data.js";
let siteData = read(siteDataPath);
const updateRegistryBlock = (page, mutate) => {
  const marker = `url: "${page}"`;
  const index = siteData.indexOf(marker);
  if (index < 0) throw new Error(`Hiányzó registry-bejegyzés: ${page}`);
  const start = siteData.lastIndexOf("\n    {", index);
  const endMarker = siteData.indexOf("\n    },", index);
  if (start < 0 || endMarker < 0) throw new Error(`Nem olvasható registry-blokk: ${page}`);
  const end = endMarker + "\n    },".length;
  const before = siteData.slice(start, end);
  const after = mutate(before);
  siteData = siteData.slice(0, start) + after + siteData.slice(end);
};

for (const page of retired) {
  updateRegistryBlock(page, (block) => /hidden:\\s*true/.test(block)
    ? block
    : block.replace(/(\\n\\s+group:\\s*"[^"]+",)/, `$1\n      hidden: true,`));
}

updateRegistryBlock(hubPath, (block) => block
...
write(siteDataPath, siteData);

const cookiePath = "js/cookie.js";
let cookie = read(cookiePath);
for (const page of retired) {
  const slug = page.replace(/\\.html$/, "");
  if (cookie.includes(`"${slug}"`)) continue;
  cookie = cookie.replace(
    /(const adsenseExcludedExact = new Set\\(\\[[\\s\\S]*?)(\\n\\s*\\]\\);)/,
    `$1\n    "${slug}",$2`
  );
}
write(cookiePath, cookie);

const adsenseAuditPath = "scripts/adsense-quality-v2-audit.js";
let adsenseAudit = read(adsenseAuditPath);
if (!adsenseAudit.includes("calculatorByPath.get(name)?.hidden === true")) {
  adsenseAudit = adsenseAudit.replace(
    "if (informationalExact.has(name) || phase2RetiredCalculators.has(name) || isRedirect(html)) return false;",
    "if (informationalExact.has(name) || phase2RetiredCalculators.has(name) || calculatorByPath.get(name)?.hidden === true || isRedirect(html)) return false;"
  );
}
write(adsenseAuditPath, adsenseAudit);

const redirectsPath = "_redirects";
let redirects = read(redirectsPath).trimEnd();
for (const page of retired) {
  const extensionless = `/${page.replace(/\\.html$/, "")} /kalkulatorok/atlag-kalkulator 301`;
  const htmlRule = `/${page} /kalkulatorok/atlag-kalkulator 301`;
